import threading
import time

from cykit.queue import BroadcastQueue, BroadcastMode

MSG_SIZE = 64
NUM_MSGS = 2_000_000

payload = "x" * MSG_SIZE


def make_payload(i):
    return f"{i:0{MSG_SIZE}d}"


payloads = [f"{i:0{MSG_SIZE}d}" for i in range(NUM_MSGS)]

q = BroadcastQueue(
    capacity=65536,
    mode=BroadcastMode.SPSC,
    block_on_full=True,
)


def producer(evt):
    evt.wait()

    t0 = time.perf_counter_ns()

    # for p in payloads:
    #    q.push(p)
    for _ in range(NUM_MSGS):
        q.push(payload)

    q.push("")

    t1 = time.perf_counter_ns()

    print(f"Producer time : {(t1-t0)/1e9:.6f} sec")


def consumer(evt):
    evt.wait()

    first = None
    count = 0

    while True:
        msg = q.pop()

        if first is None:
            first = time.perf_counter_ns()

        # if count == NUM_MSGS:
        if msg == "":
            q.close()
            break

        count += 1

    end = time.perf_counter_ns()

    elapsed = (end - first) / 1e9

    print()
    print("====== cykit.BroadcastQueue ======")
    print(f"Messages      : {count:,}")
    print(f"Elapsed       : {elapsed:.6f} sec")
    print(f"Throughput    : {count/elapsed:,.0f} msg/sec")
    print(f"Bandwidth     : {(count*MSG_SIZE)/(1024*1024)/elapsed:.2f} MiB/sec")
    print(f"Avg Latency   : {elapsed*1e6/count:.3f} us/msg")


evt = threading.Event()

tp = threading.Thread(target=producer, args=(evt,))
tc = threading.Thread(target=consumer, args=(evt,))

tp.start()
tc.start()

time.sleep(1)
evt.set()

tp.join()
tc.join()
