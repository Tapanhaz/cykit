 ...
 */
 ...
 do \
     { \
        struct timeval tv_; \
        tv_.tv_sec  = static_cast<decltype(tv_.tv_sec)>(secs); \
        tv_.tv_usec = static_cast<decltype(tv_.tv_usec)>( \
            ((secs) - static_cast<long>(secs)) * 1e6); \
 ...
 class TransportError : public std::exception {
     public:
         TransportErrorKind kind   = TransportErrorKind::NoErr;
         int               code   = 0;   
         std::string       message;
         std::string       url;          
         int               http_status = 0;
         ...
         explicit operator bool() const noexcept {
             return kind != TransportErrorKind::NoErr;
         }
 ...
 #ifdef _WIN32
 inline std::string certifi_ca_bundle_path() {
     PyGILState_STATE _gstate = PyGILState_Ensure();
     std::string out;
     PyObject* certifi_mod = PyImport_ImportModule("certifi");
     if (!certifi_mod) {
         PyErr_Clear();
         PyGILState_Release(_gstate);
         return out;
     }
     PyObject* where_fn = PyObject_GetAttrString(certifi_mod, "where");
     Py_DECREF(certifi_mod);
     if (!where_fn) {
         PyErr_Clear();
         PyGILState_Release(_gstate);
         return out;
     }
     PyObject* result = PyObject_CallObject(where_fn, nullptr);
     Py_DECREF(where_fn);
     if (!result) {
         PyErr_Clear();
         PyGILState_Release(_gstate);
 ...
 inline std::string get_ca_bundle_path() {
 #ifdef _WIN32
     static std::string _cached_bundle;
     static std::once_flag _once;
     std::call_once(_once, []() {
         _cached_bundle = certifi_ca_bundle_path();
     });
 ...
 inline ssl::context create_ssl_context(const TlsPolicy& policy = TlsPolicy{}) {
     ...
     if (policy.verify_peer) {
         ...
         #ifdef _WIN32
             std::string _bundle = get_ca_bundle_path();
             if (!_bundle.empty()) {
                 boost::system::error_code _ca_ec;
                 ctx.load_verify_file(_bundle, _ca_ec);
             }
         #endif
         }
     } else {
 ...
 inline int set_linger(socket_t sock, linger& lg)
 {
 #ifdef _WIN32
     return setsockopt(
         sock,
     ...
     void connect(const std::string& host, uint16_t port, TcpTimeouts timeouts, bool keepalive, CancelToken token = {}) {
         ...
         for (rp = res; rp; rp = rp->ai_next) {
 #ifdef _WIN32
             //fd = static_cast<RAW_SOCKET>(WSASocketW(
             ...
             if (r == 0 || in_progress) {
                 ...
                 long tv_sec_whole = static_cast<long>(timeouts.connect_sec);
                 struct timeval tv;
                 tv.tv_sec  = static_cast<decltype(tv.tv_sec)>(tv_sec_whole);
                 tv.tv_usec = static_cast<decltype(tv.tv_usec)>(
                     (timeouts.connect_sec - tv_sec_whole) * 1e6);
 
 //                long tv_sec = static_cast<long>(timeouts.connect_sec);  // macos avoids aggregate initialization
         ...
         HttpResponse do_wire_request(const std::string& method,
             ...
             try {
                 if (pu.tls) {
                     ...
                     boost::system::error_code ec;
 //                #ifdef _WIN32
 //                    struct linger lg{1, 0};   // abbortive shutdown for windows .
 //                    set_linger(beast::get_lowest_layer(stream).socket().native_handle(), lg);       
 //                #else
 ...
 struct SmtpSendResult {
     ...
     SmtpErrorClass  error_class  = SmtpErrorClass::NoErr;
     int             attempts     = 0;
 ...
 private:
     ...
     void do_auth(Stream& stream) {
         if (auth_mech_ == SmtpAuth::Off) return;
     ...
     SmtpSendResult send_to_stream(const SmtpMessage& msg) {
         ...
         if (caps_.pipelining) {
             ...
             auto send_rcpts = [&](const std::vector<std::string>& addrs)
                 ...
                 ok.smtp_code  = 250;
                 ok.smtp_message = "ok";
                 ok.error_class  = SmtpErrorClass::NoErr;
                 return ok;                            
             };
